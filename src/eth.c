...
int32_t eth_tx(uint8_t *buf, int32_t len) {
    ...
    eth_tx_descriptor[eth_tx_desc_index].ui32Count = (uint32_t) len;
    eth_tx_descriptor[eth_tx_desc_index].pvBuffer1 = buf;
...
void eth_init() {
    ...
    for (int i = 0; i < ETH_NUM_TX_DESCRIPTORS; i++) {
        ...
        eth_tx_descriptor[i].pvBuffer1 = eth_tx_buf;
    ...
    for (int i = 0; i < ETH_NUM_RX_DESCRIPTORS; i++) {
        ...
        eth_rx_descriptor[i].pvBuffer1 = eth_rx_buf;
